...
| データベース | SQLite（expo-sqlite） | 端末内保存。API連携なし |
| グラフ | react-native-chart-kit または victory-native | 週次・相手別の傾向を可視化 |
| AI / API | なし（ルールベース + 統計計算） | API費用ゼロ |
| 通知 | なし | MVPでは実装しない |
| 認証 | なし | MVPでは端末内の個人利用に限定 |
| 利用ログ | SQLiteに保存 | 記録作成・画面閲覧・グラフ閲覧などを記録 |
| フィードバック | SQLiteに保存 | 要望・不具合・感想を端末内に保存 |
